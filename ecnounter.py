...
class Encounter:
    ...
    def inv_use(self, hero_id):
        ...
        elif hero_id in self.currentEntity.get_inv():
            item_name = self.currentEntity.get_inv()[self.something].use()
            self.currentEntity.inv_remove(hero_id, False)
    ...
    def inv_give(self, acceptor, hero_id):
        if len(self.currentEntity.get_inv()) == 0:
            ...
            acceptor.inv_add(hero_id)
            self.currentEntity.inv_remove(hero_id, False)
            print("[OK] You gave", acceptor.get_name(), " ", hero_id, "!")
        else:
            print("[ER] You can’t give that!")

    def inv_equip(self, is_armor, hero_id):
        if hero_id in self.currentEntity.get_inv():
            item_class = None
            if is_armor:
                item_class = "armor"
                temp = self.currentEntity.get_armor()
                self.currentEntity.set_armor(hero_id)
                self.currentEntity.inv_remove(hero_id, False)
                self.currentEntity.inv_add(temp)
            elif not is_armor:
                item_class = "weapon"
                temp = self.currentEntity.get_weapon()
                self.currentEntity.set_weapon(hero_id)
                self.currentEntity.inv_remove(hero_id, False)
                self.currentEntity.inv_add(temp)
